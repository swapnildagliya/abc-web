...
// 20 route dirs since 2026-09-08: privacy/ is the GDPR notice.
...
/* 404.html is GENERATED by src/pages/redirects.mjs. The rescue was first written
   straight into 404.html and the next build silently erased it — the whole
   point of fixing generators rather than outputs. This checks the built file,
   so an edit that lives only in the output cannot survive a rebuild unnoticed. */
const notFound = readFileSync(join(ROOT, "404.html"), "utf8");
check("404 carries the case-insensitive rescue", notFound.includes("routes.json") && notFound.includes("location.replace"));
check("the rescue is in the generator, not just the output",
  readFileSync(join(ROOT, "src/pages/redirects.mjs"), "utf8").includes("routes.json"));
